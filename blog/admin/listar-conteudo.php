...
if (isset($_GET['action-post']) && isset($_GET['id']) && !empty($_GET['action-post']) && !empty($_GET['id'])) {
  ...
  switch ($_GET['action-post']) {
    case 1:
    Update('postagem', array('statuspost' => 0), "idpost = '{$id}'");
    break;
    case 2:
    Update('postagem', array('statuspost' => 1), "idpost = '{$id}'");
    break;
  }
  header('Location: listar-conteudo.php');
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="jumbotron">
                <h1 class="display-3">Listar Conteúdos Postados</h1>
                <p class="lead">Selecione os conteúdos que serão alterados, em caso de atualização de informações. Observe que, nenhuma notícia ou artigo será excluído. Apenas desativado no blog principal.</p>
                <hr class="m-y-md">
            </div>
            ...
            <table id="" class="display table table-responsive table-striped">
                ...
                <tbody>
                    <?php
                    $conteudo = Read('postagem', 'order by idpost desc');
                    if (!$conteudo) {
                        ...
                        foreach ($conteudo as $cont) {
                            ?>
                            <tr>
                                <td><?php echo $cont['idpost']; ?></td>
                                <td>
                                    <?php
                                    $str = strip_tags(stripslashes($cont['titulo']));
                                    $len = strlen($str);
                                    $max = 50;
                                ...
                                <td><?php echo date('d/m/Y', strtotime($cont['data'])); ?></td>
                                <td><?php echo $cont['tipopostagem']; ?></td>
                                <?php
                                if ($cont['statuspost'] == 1) {
                                    echo "<td>Ativo</td>";
                                } else if ($cont['statuspost'] == 0) {
                                    echo "<td>Inativo</td>";
                                }
                                ?>
                                <td>
                                    <a href="editar-conteudo.php?id=<?php echo $cont['idpost']; ?>" target="_blank" class="btn btn-primary">Editar</a>
                                    <?php
                                    if ($cont['statuspost'] == 1) {
                                        ?>
                                        <a href="?action-post=1&&id=<?php echo $cont['idpost']; ?>" class="btn btn-danger">Desativar</a>
                                        <?php
                                    } else {
                                        ?>
                                        <a href="?action-post=2&&id=<?php echo $cont['idpost']; ?>" class="btn btn-success">Ativar</a>
                                        <?php
                                    }
                                    ?>
                                </td>
